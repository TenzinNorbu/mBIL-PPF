...
class RefundProcessController extends Controller
{
    ...
    public function refundProcess(Request $request)
    {
        DB::beginTransaction();
        $refund = new Refund();
        $refundRefNo = 'REF-PF' . date('Ymd') . Carbon::now()->format('His') . random_int(2222222222, 9999999999);
        ...
        $refund_employee_cont = $request->refund_emplyee_contrib;
        $refund_employer_cont = $request->refund_emplyr_contrib;
        ...
        $refund_process_employee = (float)$refund_employee_cont;
        $refund_process_employer = (float)$refund_employer_cont;
        ...
        $total_prev_disbursed_employee = (float)$total_prev_disbursed_amount / 2;  //can get exact figure from database
        $total_prev_disbursed_employer = (float)$total_prev_disbursed_amount / 2;
        ...
        $refund_gross_amount = ($total_gross_amount + $as_on_date_interest_1 + $as_on_date_interest_2);

        if (round($refund_gross_amount,0) < round($total_refund_amount,0)) {
        ...
        if ($overall_refund_amount_employer < (float)$total_accumulated_employer_contribution) {
            ...
            $refundable_interest_employer = $overall_refund_amount_employer - ((float)$total_accumulated_employer_contribution); // 0.81
        ...
        $refund->refund_employee_contribution = (float)$total_accumulated_employee_contribution;
        $refund->refund_employer_contribution = (float)$total_accumulated_employer_contribution;

        $refund->refund_interest_on_employee_contr = $refundable_interest_employee - (float)$as_on_date_interest_1;
        $refund->refund_interest_on_employer_contr = $refundable_interest_employer - (float)$as_on_date_interest_2;
        ...
        $total_refund_contribution = $refundable_contribution_employee + $refundable_contribution_employer;
        $total_refund_interest = $refundable_interest_employee + $refundable_interest_employer;
        ...
        $refund->registration_type = 'PF';

        $refund->refund_bank_name = $request->bank_name;
        $refund->refund_bank_account_no = $request->pf_emp_acc_no;
        ...
        $emp_data = Pfemployeeregistration::where('pf_employee_id', '=', $request->refund_employee_id)
            ->where('effective_end_date', '=', NULL)
            ->where('registration_type', '=', 'PF')->get()->first();
            ...
            foreach ($contactMailList as $contact_data) {
                foreach ($proprietorMailList as $proprietor_data) {
                    ...
                    try {
                        ...
                        Mail::send('emails.refundprocessmail', $details, function ($message) use ($contactEmail, $proprietor_email) {
        ...
        if ($refund->save()) {
            ...
            if ($this->saveintopfindividualdeposit($request, $refundRefNo, $forTheYear, $forTheMonth, $as_on_date_interest_1,
                ...
                if (((int)$gross_amount_1 <= 10 && (int)$gross_amount_2 <= 10) || ((int)$net_amount_1 <= 10 && (int)$net_amount_2 <= 10)) {
                                                        ...
                                                        $companyRefNo, $empRefId)
    {
        ...
        $accountTransaction->account_voucher_narration = 'Refund by the PF Dept against the Refund-Ref-No : ' . $refundRefNo;
        ...
        $getAccountTransactionSerialNo = DB::table('masteridholders')
            ...
            ->where('masteridholders.registration_type', '=', 'PF')
        ...
        $voucher_number = 'SYS/PF/' . $year . '/' . $branch_code . '/' . $accountTransactionSerialNo;
                                                               ...
                                                               $companyRefNo, $empRefId, $ref_branch_id)
    {
        ...
        foreach ($transactionCategorySave as $transactionId) {
            ...
            if ($transactionId == 'InterestPaid') {
                ...
                $saveAccountTransactionDetail->acc_narration = 'PF Interest Payable DR against the Refund-No :  ' . $refundRefNo;
            }
                                                           ...
                                                           $companyRefNo, $empRefId)
    {
        ...
        $accountTransaction->account_voucher_narration = 'PF Refund Payment against Refund-No : ' . $refundRefNo;
        ...
        $accountTransactionSerialNo = (int)$getAccountTransactionSerialNo->serial_no + 1;
        $branch_code = $getAccountTransactionSerialNo->branch_code;
        $voucher_number = 'Refund/PF/' . $currentYear . '/' . $branch_code . '/' . $accountTransactionSerialNo;
                                                                 ...
                                                                 $refundTotalInterest, $companyRefNo, $empRefId)
    {
        ...
        foreach ($transactionCategory as $transaction) {
            ...
            if ($transaction == 'PfcollectionAcc') {
                ...
                $saveAccountTransactionDetail->acc_credit_amount = 0;
                $saveAccountTransactionDetail->acc_narration = 'PF Refund contribution against Refund-No : ' . $refundRefNo;
                ...
                $saveAccountTransactionDetail->acc_account_type_id = 'A5C730C0-45DF-11EC-AAE3-1D943BCC8402';
                $saveAccountTransactionDetail->acc_debit_amount = 0;
                $saveAccountTransactionDetail->acc_credit_amount = (float)$refundTotalContribution;
                $saveAccountTransactionDetail->acc_narration = 'PF Refund contribution payable against Refund-No : ' . $refundRefNo;

            } elseif ($transaction == 'PfInterestRefundPayableAcc') {

                $saveAccountTransactionDetail->acc_account_group_id = '987A9B20-45DE-11EC-973C-47DC726D5DD3';
                $saveAccountTransactionDetail->acc_account_type_id = 'B68C9880-45DF-11EC-B0EC-4568E1C45C9C';
                $saveAccountTransactionDetail->acc_debit_amount = 0;
                $saveAccountTransactionDetail->acc_credit_amount = (float)$refundTotalInterest;
                $saveAccountTransactionDetail->acc_narration = 'PF Refund Interest against Refund-No : ' . $refundRefNo;
                ...
                $saveAccountTransactionDetail->acc_credit_amount = 0;
                $saveAccountTransactionDetail->acc_narration = 'PF Refund Interest payable against Refund-No ' . $refundRefNo;
            }
    ...
    public function refundProcessFileUpload(Request $request, $refundRefNo)
    {
        ...
        $filename = 'pf_refund_process_upload_' . $refundRefNo . '_' . $original_file_name;
    ...
    public function createRefundProcessedDocument(Request $request, $refundRefNo, $refund_remarks, $userName, $userBranch, $userBranch_id)
    {
        ...
        $refundRef_No = 'BIL/PF/' . $year . '/RF/' . $refund_serial_no;
        ...
        $bladeView = view('refundfiles.refundprocess', $data);
        ...
        $fileName = 'pf_refund_process_slip_' . $genRefundSlipExtensionNo . '_' . $currentDateTime . '.pdf';
        ...
        if ($pdf->save(Storage::disk('refundslip')->put($fileName, $pdf->output()))) {
            ...
            $save_doc_data = DB::table('documents')->insert([
                ...
                'document_type' => 'RefundProcessDoc',
