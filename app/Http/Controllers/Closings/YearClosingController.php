<?php
namespace App\Http\Controllers\Closings;
...
{
    public function yearClosing(Request $request) {
        $closing_date = $request->closing_date;
        $getForTheMonth = $request->closing_month;
        $getForTheYear = $request->closing_year;

        $getActiveCompanyList = Companyregistration::with('getPfMouDetails')
        ...
        foreach ($getActiveCompanyList as $cmp_data) {
            ...
            foreach ($get_active_employee_list as $emp_data) {
                ...
                if ($get_transaction_details != '' && $get_transaction_details != NULL) {
                    ...
                    $ref_interest_rate = $cmp_data->getPfMouDetails->interest_rate;
                    ...
                    $no_of_days = round(((strtotime($closing_date) - strtotime($get_transaction_details->transaction_date)) / 24 / 3600), 0) + 1;
                         ...
                         if ($this->saveStatementClosingTags($companyId, $employeeId, $closing_date, 
                            $ref_branch_id, $getForTheMonth,$getForTheYear, $ref_interest_rate, 
                            $employeeContribution, $employerContribution, $totalEmployeeContribution,
                            $totalEmployerContribution, $totalInterestOnEmployee, $totalInterestOnEmployer, 
                            $interest_chargeable_amount01,$interest_chargeable_amount02, 
                            $gross_os_balance_employee, $gross_os_balance_employer, $employeeName,
                            $no_of_days) == 'error') {
    ...
    public function saveStatementClosingTags($companyId, $employeeId, $closing_date, $ref_branch_id,
                    $getForTheMonth, $getForTheYear, $ref_interest_rate, $employeeContribution, 
                    $employerContribution,$totalEmployeeContribution, $totalEmployerContribution, 
                    $totalInterestOnEmployee,$totalInterestOnEmployer, $interest_chargeable_amount01, 
                    $interest_chargeable_amount02,$gross_os_balance_employee, $gross_os_balance_employer, 
                    $employeeName, $no_of_days) 
    {
        $transaction_ref_no = date('YmdH') . random_int(333333333, 999999999);
        $interestAccruedEmployee = ($interest_chargeable_amount01 * (float)$ref_interest_rate / 100 * $no_of_days) / 365;
        $interestAccruedEmployer = ($interest_chargeable_amount02 * (float)$ref_interest_rate / 100 * $no_of_days) / 365;
        ...
        $pf_deposit->interest_accrued_employee_contribution = (float)$interestAccruedEmployee;
        $pf_deposit->interest_accrued_employer_contribution = (float)$interestAccruedEmployer;
        ...
        $pf_deposit->registration_type = 'PF';
         ...
         if (!$pf_deposit->save()) {
            ...
            if ($this->closingInterestAccountTransactions($transaction_ref_no, $companyId, $employeeId, $closing_date, $ref_branch_id,
                ...
                if ($this->saveStatementOpeningTags($companyId, $employeeId, $transaction_ref_no, $closing_date, $getForTheMonth,$getForTheYear, $totalEmployeeContribution, $totalEmployerContribution, 
                    $int_outstanding_01, $int_outstanding_02,$int_chargeable_amt_1, $int_chargeable_amt_2, $outstanding_01, $outstanding_02, $ref_interest_rate,
    ...
    public function saveStatementOpeningTags($companyId, $employeeId, $transaction_ref_no, $closing_date,
        $getForTheMonth, $getForTheYear,$totalEmployeeContribution, $totalEmployerContribution, 
        $int_outstanding_01, $int_outstanding_02,$int_chargeable_amt_1, $int_chargeable_amt_2,
        ...
        $pf_deposit->for_the_month = 1;
        $pf_deposit->for_the_year = (int)$getForTheYear + 1; // changes here
        $pf_deposit->employee_contribution = 0;
        $pf_deposit->employer_contribution = 0;
        $pf_deposit->total_employee_contribution = $totalEmployeeContribution;
        $pf_deposit->total_employer_contribution = $totalEmployerContribution;
        ...
        $pf_deposit->interest_chargeable_amount_1 = (float)$outstanding_01;
        $pf_deposit->interest_chargeable_amount_2 = (float)$outstanding_02;
    ...
    public function closingInterestAccountTransactions($transaction_ref_no, $companyId, $employeeId, $closing_date, $ref_branch_id, $getForTheYear,$interestAccruedEmployee, $interestAccruedEmployer, 
        $employeeName)
    {

        $accountTransaction = new Accounttransaction();
        $accountTransactionId = date('YmdH') . random_int(11111111, 99999999);
        $accountTransaction->account_transaction_id = $accountTransactionId;
        $accountTransaction->account_voucher_type = 'Closing';
        $accountTransaction->account_voucher_date = $closing_date;
        ...
        $accountTransaction->account_voucher_narration = 'PF Year Closing interest accrued payable against employee account no: ' . $employeeId;

        $getAccountTransactionSerialNo = DB::table('masteridholders')
            ->join('branches', 'branches.id', '=', 'masteridholders.branch_id')
            ->where('id_type', '=', 'Sys_Transaction')
        ...
        $cheque_date = NULL;
        $col_bank_name = NULL;
        $voucher_number = 'CL/PF/' . date('Y') . '/' . $branch_code . '/YC/SYS/' . $accountTransactionSerialNo;
    ...
    public function closingInterestAccountTransactionDetail($accountTransactionId, $transaction_ref_no, 
        $companyId, $employeeId,$closing_date, $interestAccruedEmployee,
        $interestAccruedEmployer, $employeeName)
    {
        ...
        foreach ($closing_category as $closing_data) {
            ...
            $genAccountTransactiondetailsId =  Carbon::now()->format('YmdH'). random_int(11111111, 99999999);
            ...
            if ($closing_data == 'PPFInterestPaidAccount') {
                ...
                $saveAccountTransactionDetail->acc_narration = 'PF Interest Paid  : ' . $employeeName;
                ...
                $saveAccountTransactionDetail->acc_narration = 'PF Interest Payable  :  ' . $employeeName;
            }
