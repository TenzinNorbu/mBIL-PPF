...
class GfRefundApproveController extends Controller
{
    ...
    public function SaveRefundApprovalData(Request $request)
    {
        ...
        foreach ($anotherArray as $payment_data) {
            ...
            $refundApproveId = random_int(6666666, 9999999);
            ...
            $refundApprovePayment->registration_type = 'GF';
            ...
            foreach ($data as $datas) {
                ...
                $contactMailList = Contactperson::where('contact_person_company_id', '=', $paymentCompanyId)
                    ...
                    ->where('registration_type', '=', 'GF')->get();
                ...
                $refund_data = Refund::where('refund_employee_id', '=', $employeeID)
                    ->where('registration_type', '=', 'GF')
                    ->where('refund_ref_no', '=',$datas['refund_ref_no'])
                    ->get()->first();
                    ...
                    foreach ($contactMailList as $contact_data) {
                        foreach ($proprietorMailList as $proprietor_data) {
                            ...
                            try {
                                ...
                                Mail::send('emails.gf_refundapprovemail', $details, function ($message) use ($contactEmail, $proprietor_email) {
            ...
            if (!$refundApprovePayment->save()) {
                DB::rollBack();
                return response()->json(['error', 'message' => 'GF Refund Approve Failed!']);
    ...
    public function SavePaymentDetails(Request $request, $refundApproveId, $paymentCompanyId)
    {
        ...
        foreach ($data as $payment_details_data) {
            ...
            if ($payment_details_data['company_id'] == $paymentCompanyId) {
                ...
                $refundPaymentApproveDetails->registration_type = 'GF';
    ...
    public function createRefundApprovalDocument($get_data, $refundApproveId, $paymentCompanyId, $total_payable_amount, $employeeName, $refund_data)
    {
        ...
        $refundRefNo = 'BIL/GF/' . Carbon::now()->year . '/RF';

        $orgName = Companyregistration::where('company_id', $paymentCompanyId)
            ->get()->first()->org_name;
        ...
        $bladeView = view('refundfiles.gf_refundapprove', compact('get_data', 'refundRefNo', 'paymentCompanyId',
        ...
        $fileName = 'gf_refund_payment_advise_note_' . $genRefundSlipExtensionNo . '_' . $currentDateTime . '.pdf';
