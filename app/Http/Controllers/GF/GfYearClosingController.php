...
class GFYearClosingController extends Controller
{
    public function yearClosing(Request $request) {
        ...
        foreach ($getActiveCompanyList as $cmp_data) {
            ...
            foreach ($get_active_employee_list as $emp_data) {
                ...
                if ($get_transaction_details != '' && $get_transaction_details != NULL) {
                    ...
                    if ($this->saveGFStatementClosingTags($companyId, $employeeId, $closing_date, $ref_branch_id, $getForTheMonth,
      ...
      public function saveGFStatementClosingTags($companyId, $employeeId, $closing_date, $ref_branch_id, $getForTheMonth,
                                               ...
                                               $gross_os_balance_employee, $gross_os_balance_employer, $employeeName, $no_of_days)
    {
        ...
        if (!$pf_deposit->save()) {
            ...
            if ($this->closingInterestAccountTransactions($transaction_ref_no, $companyId, $employeeId, $closing_date, $ref_branch_id,
                ...
                if ($this->saveGFStatementOpeningTags($companyId, $employeeId, $transaction_ref_no, $closing_date, $getForTheMonth,
    ...
    //  Opening Tag begins
    public function saveGFStatementOpeningTags($companyId, $employeeId, $transaction_ref_no, $closing_date, $getForTheMonth, $getForTheYear,
                                               ...
                                               $outstanding_01, $outstanding_02, $ref_interest_rate, $transaction_version_no)
    {
        ...
        $pf_deposit->interest_chargeable_amount_1 = (float)$outstanding_01; // int_chargeable_amt_1
        $pf_deposit->interest_chargeable_amount_2 = (float)$outstanding_02; // int_chargeable_amt_2

        $pf_deposit->gross_os_balance_employee = (float)$outstanding_01; // outstanding_01
        $pf_deposit->gross_os_balance_employer = (float)$outstanding_02; // outstanding_02  
                                                       ...
                                                       $getForTheYear, $interestAccruedEmployee, $interestAccruedEmployer, $employeeName)
    {
        ...
        $accountTransaction->account_voucher_amount = (float)$interestAccruedEmployee + (float)
        $interestAccruedEmployer;
        $accountTransaction->account_voucher_narration = 'GF Year Closing interest accrued payable against employee account no: ' . $employeeId;
        ...
        $cheque_date = NULL;
        $col_bank_name = NULL;
        $voucher_number = 'CL/GF/' . date('Y') . '/' . $branch_code . '/YC/SYS/' . $accountTransactionSerialNo;
