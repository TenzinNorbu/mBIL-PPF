...
<div>
    ...
    <table class="table table-bordered" style="border-collapse: collapse; border: none;">
        ...
        <tr>
            <th colspan="2" style="border: none;"><h2>PF REFUND PAYMENT ADVISE</h2></th>
        </tr>
        <tr>
        ...
        <tr style="border: none;">
            <td>
                <p style="text-align: justify">
                    ...
                    Subject: <b>PF Refund Disbursement Advise</b><br><br>

                    Please kindly arrange to refund the sum of Nu.{{ number_format($total_payable_amount,2) }}/- (
        ...
        @foreach($get_data as $key=>$approve_data)
            <tr>
                @if($approve_data['company_id'] == $paymentCompanyId)
                    ...
                    $getEmpData = \App\Models\Pfemployeeregistration::where('pf_employee_id', '=', $EmpId)->get();
                    $employee_cid = $getEmpData->first()->identification_no;
                    $employee_name = $getEmpData->first()->employee_name;

                    $total_refund_contribution = $total_refund_contribution + (float)$approve_data['refund_total_contr'] + (float)$approve_data['refund_total_interest'];
